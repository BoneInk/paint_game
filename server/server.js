...
io.sockets.on('connection', function (socket) {
    socket.on('login', function (name) {
        this.name = name || "无名游客";
        this.attrin = false;
        this.emit('server msg', '您当前的昵称为： ' + this.name + ' !');
        this.broadcast.emit('server msg', '欢迎 ' + this.name + ' 进入房间!');
        this.emit('paint paths', JSON.stringify(paths));
        var users = Game.inQueue.map(x => {
            return getSocket(x)
        });
        ...
        this.on('out', function () {
            ...
            Game.inQueue.splice(Game.inQueue.findIndex(x => {
                x.id === this.id;
            }));
        ...
        this.on('paint tool',function (tool_num,sX,sY,eX,eY) {
            this.broadcast.emit('paint tool',tool_num,sX,sY,eX,eY);
        })
